...
function AutocompleteInMemoryFormItem(props) {
  ...
  const prioritizeFromDataSource = (selected) => {
    return (
      (fullDataSource || []).find((item) => item.value === selected.value) ||
      selected
    );
  };
  ...
  const valueMultiple = () => {
    if (originalValue) {
      return originalValue.map((value) =>
        prioritizeFromDataSource(mapper.toAutocomplete(value))
      );
    }
  ...
  return (
    <div className='form-group'>
      ...
      <div style={{ display: "flex" }}>
        ...
        {election && props.showCreate && props.hasPermissionToCreate ? (
          <button
